...
dfs(root, 0, tree, parents, depths)
...
def get_lca(node1, node2, parents, depths):
...
for node1, node2, node3 in querys:
    lca = get_lca(node1, node2, parents, depths)
    lca = get_lca(lca, node3, parents, depths)
    print(lca)
